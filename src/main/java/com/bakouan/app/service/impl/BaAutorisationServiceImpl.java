...
public class BaAutorisationServiceImpl implements BaAutorisationService {
    ...
    public BaAutorisationSpecialeDto create(final BaAutorisationSpecialeDto autorisationSpecialeDto, final MultipartFile noteVerbale) {
        log.info("Création d'une autorisation spéciale par : {}", autorisationSpecialeDto.getId());

        // Enregistrer la note verbale
    ...
    /**
     * Service d'ajout de document à une autorisation spéciale
     *
     * @param autorisationId : id de l'autorisation speciale
     * @param noteVerbale    : le document
     * @param noteVerbale    : la note verbale
     * @return : le dto de l'autorisation speciale
     */
    ...
    public BaAutorisationSpecialeDto uploadNoteVerbale(final String autorisationId, final MultipartFile noteVerbale) {
        ...
        // Trouver l'autorisation par ID
        BaAutorisationSpeciale autorisation = autorisationRepository.findById(autorisationId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Autorisation spéciale introuvable."));
    ...
    /**
     * Service de suppression de la note verbale à une autorisation spéciale
     *
     * @param autorisationId: id de l'autorisation speciale
     * @param documentId      : id de la note verbale
     * @return : le dto de l'autorisation speciale
     */
    ...
    public BaAutorisationSpecialeDto removeNoteVerbale(final String autorisationId, final String documentId) {
        ...
        // Vérifier que le document est bien lié à l'autorisation
    ...
    /**
     * Service de mise à jour d'une autorisation spéciale
     * @param id    : id de l'autorisation speciale
     * @param autorisationSpecialeDto : le dto de l'autorisation speciale
     * @return  : le dto de l'autorisation speciale
     */
    ...
    public BaAutorisationSpecialeDto update(String id, final BaAutorisationSpecialeDto autorisationSpecialeDto) {
        ...
        BaAutorisationSpeciale existingAutorisation = autorisationRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Autorisation introuvable"));

        // Récupération de la mission diplomatique (si nécessaire)
        ...
        // Sauvegarde de l'autorisation mise à jour
        BaAutorisationSpeciale updatedAutorisation = autorisationRepository.save(existingAutorisation);

        // Retourner la version mise à jour sous forme de DTO
        return mapper.maps(updatedAutorisation);
    }

    /**
     * Service de suppression (Archivage) d'une autorisation spéciale
     * @param id : id de l'autorisation speciale
     */
    ...
    public BaAutorisationSpecialeDto validateSt(String id) {
        logService.log(new BaLogDto(EAction.U, "Recherche de l'autorisation spéciale par le Service technique ID : {}"+ id));
        BaAutorisationSpeciale autorisation = autorisationRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Autorisation introuvable"));
    ...
    public BaAutorisationSpecialeDto validateDg(String id) {
        logService.log(new BaLogDto(EAction.U, "Valider de l'autorisation spéciale par le DGID : {}"+ id));
        BaAutorisationSpeciale autorisation = autorisationRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Autorisation introuvable"));

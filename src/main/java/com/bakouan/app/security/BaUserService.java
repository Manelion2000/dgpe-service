...
public class BaUserService {
     ...
     * Récupère la liste des utilisateurs en fonction de leur statut admin.
     *
     * @param isDiplomate True pour récupérer les administrateurs, False pour les non-administrateurs.
     * @return Liste des utilisateurs filtrés.
     */
    public List<BaUserDto> getUsersByMissionDiplomatique(boolean isDiplomate) {
        return userRepository.findByIsDiplomate(isDiplomate)
                .stream()
                .map(mapper::maps) // Conversion en DTO
                .collect(Collectors.toList());
    }

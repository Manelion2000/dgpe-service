...
public interface YtMapper {
    ...
    @InheritInverseConfiguration
    BaAutorisationSpeciale maps(BaAutorisationSpecialeDto dto);
    /**Gestion de delegations*/

    @Mappings({
            @Mapping(target = "documents", source = "documents")
    })
    BaDelegationMembreDto maps(BaDelegationMembre entity);

    @InheritInverseConfiguration
    BaDelegationMembre maps(BaDelegationMembreDto dto);
    /**
     * Gestion des documents de l'autorisation spéciale pour le membre de la delegation
     */
    @Mappings({
            @Mapping(target = "idMembre", source = "membre.id")
    })
    BaDocumentPersonnelAutorisationSpecialDto maps(BaDocumentPersonnelAutorisationSpecial entity);

    @InheritInverseConfiguration
    BaDocumentPersonnelAutorisationSpecial maps(BaDocumentPersonnelAutorisationSpecialDto dto);

    /**
     * Convertir une entité Document de l'autorisation spéciale en DTO.
     *
     * @param entity
     * @return le dto
     */
    @Mappings({
            @Mapping(target = "idAutorisationSpeciale", source = "autorisationSpeciale.id"),
    })
    BaDocumentAutorisationSpecialDto maps(BaDocumentAutorisationSpecial entity);
